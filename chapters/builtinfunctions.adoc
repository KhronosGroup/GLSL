...
[source,glsl]
----
genFType clamp(genFType x, genFType _minVal_, genFType maxVal)
genFType clamp(genFType x, float _minVal_, float maxVal)
ifdef::GLSL[]
...
ivec2 textureSize(gsamler2DRet _samler)
ivec2 textureSize(sampler2DectShadw _samler)
...
gvec4 texture(gsampler2D sampler, vec2 P [, float _bias])
gvec4 texture(gsampler3D sampler, vec3 P [, float _bias])
gvec4 texture(gsamplerCube sampler, vec3 P [, float _bias])
ifdef::GLSL[]
...
ifdef::GLSL[]
gvtextureGradOffset(gsampler1DArray sampler, vec2 P, float dPdx, float dPdy, int offset)
...
ifdef::GLSL[]
gvec4 textureGatherOffset(gsampler2DRect sampler, vec2 P, ivec2 offset [, int comp_])
...
gvec4 textureGatherOffsets(gsampler2DArray sampler, vec3 P, ivec2 offsets[4] [, int comp_])
...
[source,glsl]
----
int atomicAdd(inout uint mem, uint data)
int atomicAdd(inout int_mem, int data)
----
